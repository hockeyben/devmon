...
must_haves:
  ...
  key_links:
    - from: "src/devmon/models/creature.py"
      to: "src/devmon/data/creatures/*.json"
      via: "Ability model validates JSON abilities arrays"
      pattern: "abilities.*learn_level"
---

<objective>
Test scaffold, Ability model, schema v6 migration, and party field.

Purpose: Establish the model foundation for Phase 6 -- Ability model in creature.py, GameState v6 with party field, schema migration, and xfail test stubs for all battle/capture requirements. Creature JSON data updates are in Plan 02.

Output: test_battle.py with xfail stubs, Ability model in creature.py, GameState v6 with party field, _migrate_5_to_6.
</objective>
...
<task type="auto">
  <name>Task 1: Ability model, schema v6, and migration</name>
  <files>
    src/devmon/models/creature.py,
    src/devmon/models/state.py,
    src/devmon/persistence/migrations.py,
...
**4. Update test_creatures.py**: Change schema_version assertion from 5 to 6 (the test that asserts `schema_version == 5`).
  ...
  <done>Ability model exists, GameState is v6 with party field, migration works, existing creature tests pass.</done>
</task>
